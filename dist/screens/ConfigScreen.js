import { patientData, getAdherence, getActiveCount, getNotificationSettings, updateNotificationSetting, getMedications, deleteMedication } from '../data/mockData.js';
    ...
    onMount() {
        ...
        const openModal = (type, data) => {
            if (modalOverlay && modalTitle && modalContent && btnConfirmModal && btnCloseModal) {
                ...
                btnConfirmModal.style.opacity = '0.5';
                btnConfirmModal.style.cursor = 'not-allowed';
                btnConfirmModal.style.backgroundColor = '';
                btnConfirmModal.style.color = '';
                btnConfirmModal.style.border = 'none';
                ...
                if (type === 'profile') {
                    ...
                    modalContent.innerHTML = `
                        <div class="form-group"><label>Nombre</label><input type="text" class="wire-input" disabled style="${disabledStyle}" value="${(data === null || data === void 0 ? void 0 : data.name) || ''}" placeholder="Ej. Ibuprofeno"></div>
                        <div class="form-group"><label>Dosis</label><input type="text" class="wire-input" disabled style="${disabledStyle}" value="${(data === null || data === void 0 ? void 0 : data.dose) || ''}" placeholder="Ej. 400mg"></div>
                        <div class="form-group"><label>Frecuencia</label><input type="text" class="wire-input" disabled style="${disabledStyle}" value="${(data === null || data === void 0 ? void 0 : data.freq) || ''}" placeholder="Ej. 8 horas"></div>
                    `;
                }
                else if (type === 'delete') {
                    modalTitle.textContent = 'Eliminar Medicamento';
                    modalContent.innerHTML = `
                        <p style="color: var(--text-sec);">¿Estás seguro de que deseas eliminar <strong>${data === null || data === void 0 ? void 0 : data.name}</strong> de la lista?</p>
                    ...
                    btnConfirmModal.style.backgroundColor = 'var(--danger)';
                    btnConfirmModal.style.border = 'none'; // Quitar borde
                    btnConfirmModal.style.color = 'white';
                    btnConfirmModal.disabled = false;
                    btnConfirmModal.style.opacity = '1';
                    btnConfirmModal.style.cursor = 'pointer';
                    btnCloseModal.textContent = 'Cancelar';
                    pendingAction = { type: 'delete', id: data === null || data === void 0 ? void 0 : data.id };
                }
                else if (type === 'logout') {
        ...
        if (editPatientBtn)
            editPatientBtn.addEventListener('click', () => openModal('patient'));
        const addMedBtn = document.querySelector('.btn-add-med');
        if (addMedBtn)
            addMedBtn.addEventListener('click', () => openModal('med'));
